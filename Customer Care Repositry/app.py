...
def register_account():
    if request.method == "POST":
        ...
        mysql.connection.commit()
        msg = Message('Thanks for Registering',sender=config.email,
            recipients=[email]
        )
        msg.body = '''
        Welcome Aboard, we are glad you are part of our company! 
        you have succesfully created an account for Customer Care Registry.
...
def ticket_detail(id):
    ...
    if request.method == "POST":
        ...
        msg.body = f'''
            Dear Customer,
            The progress of your Ticket has been Updated and
    ...
    if user_details[3] != 2:
        ...
        if request.method == "POST":
            ...
            msg.body = """
                ...
                Now, you will be able to handle tickets for our customers.
                Congratulations!
            """
            mail.send(msg)
            return redirect(url_for("panel"))
...
def accept(ticket_id,user_id):
    ...
    if agent[4] == ticket[2]:
        ...
        mysql.connection.commit()
        msg = Message('Progress of your Ticket',sender=config.email,recipients=[customer[0]])
        msg.body = f"""
            Dear User,
...
def delete(ticket_id,user_id):
    ...
    if agent[4] == ticket[2]:
        ...
        msg.body = f"""
            ...
            Thank You for using Customer Care Registry :)
        """
        mail.send(msg)
    return redirect(url_for("home"))
